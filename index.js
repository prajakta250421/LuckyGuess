...
document.getElementById("claim-tokens").addEventListener("click", async () => {
    try {
        await memeCoinContract.methods.claimTokens(web3.utils.toWei("100", "ether")).send({ from: playerAddress });
        alert("100 TBC Tokens claimed successfully!");
...
const submitGuess = async () => {
    ...
    try {
        ...
        await memeCoinContract.methods.approve(contractAddress, web3.utils.toWei("10", "ether"))
            .send({ from: playerAddress });

        // Submit the guess and get the transaction receipt
        const receipt = await contract.methods.guessNumber(guess).send({ from: playerAddress });
        ...
        const feedback = receipt.events.GuessMade ? receipt.events.GuessMade.returnValues.feedback : "No feedback found in receipt";
        ...
        document.getElementById("feedback").textContent = "Error submitting guess.";
    }
};
