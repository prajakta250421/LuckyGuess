# LuckyGuess

A blockchain-powered interactive game where players guess a randomly chosen number between 1 and 100 to win **THEBESTCOIN(TBC)**.

## Features

- **Blockchain Integration**: Built using smart contracts on Ethereum-compatible networks.
- **Meme Coin Rewards**: Players are rewarded with THEBESTCOIN for correct guesses.
- **Responsive UI**: Mobile-friendly design with intuitive user experience.

## Steps to Play

1. **Connect Wallet**: Start the game by connecting your crypto wallet.
2. **Claim Tokens**: Receive 100 free THEBESTCOIN as an initial balance.
3. **Make a Guess**: Enter a number between 1 and 100 to play. Entry fee: 10 THEBESTCOIN.
4. **Receive Feedback**: Real-time feedback on whether your guess is too low, too high, or correct.
5. **Win Rewards**: Earn 10 THEBESTCOIN as a reward for guessing the correct number.

## Contract Details

- **Contract Address**: `0x2C1f3be15F1A081Cd2Aad1dAe744417304D13023`
- **MEME COIN Address**: `0xab2A58f7cDA5e426C18beDf518d8833822CA4656`
- **Network**: Neo X Testnet T4

## Deployment

- **Contract**: [View on Explorer](https://xt4scan.ngd.network/address/0x2C1f3be15F1A081Cd2Aad1dAe744417304D13023)
- **THEBESTCOIN**: [View on Explorer](https://xt4scan.ngd.network/token/0xab2A58f7cDA5e426C18beDf518d8833822CA4656)
- **Coin Used**: GAS

## Progress During Development

During development, we:

- Designed and deployed a robust smart contract on NeoX Testnet T4.
- Built an interactive and mobile-friendly frontend with wallet connectivity.
- Created a dynamic reward system integrating blockchain mechanics.
- Optimized UI/UX with polished styling and responsive elements.
- Documented the project with clear instructions and visuals for user engagement.

---

## Screenshots
<img width="1440" alt="Screenshot 2024-12-24 at 8 59 49 PM" src="https://github.com/user-attachments/assets/2b0366da-d14f-490f-b3ab-fddb9cce2d28" />

<img width="1440" alt="Screenshot 2024-12-24 at 8 29 23 PM" src="https://github.com/user-attachments/assets/cde72ae2-4836-4ae1-97e1-9beb8cb18229" />

Enjoy playing **LuckyGuess** and earn exciting THEBESTCOIN rewards!

